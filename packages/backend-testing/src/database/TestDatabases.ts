 ...
 */
export type TestDatabaseId = keyof typeof supportedDatabases;
  ...
  async init(id: TestDatabaseId): Promise<Knex> {
    ...
    if (!instance) {
      switch (properties.driver) {
        case 'pg':
          instance = await this.initPostgres(properties);
          break;
        case 'mysql2':
          instance = await this.initMysql(properties);
          break;
        case 'sqlite3':
          instance = await this.initSqlite(properties);
          break;
        default:
